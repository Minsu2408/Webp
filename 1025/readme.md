...
## 강의 목표
- 이벤트 개념 이해 및 이벤트 리스너 작성법 습득
- DOM 트리 따라 이벤트 흐름 이해
- onload, 폼, 마우스, 키 이벤트 다루기

## 이벤트 개요
- 이벤트: 사용자 입력 및 문서, 브라우저 상태 변화 통지
- 이벤트 리스너: 이벤트 대처용 자바스크립트 코드
- 주요 이벤트: click, dblclick, keydown, load, submit, reset 등
...
## 이벤트 객체
- 이벤트 발생 정보 포함 (타입, 타겟 등)
- 이벤트 리스너 매개변수로 전달
- 기본 동작 취소: `preventDefault()`, `return false`

## 이벤트 흐름
- 캡처링 (window → 타겟)
- 버블링 (타겟 → window)
- 흐름 중단: `stopPropagation()`
- addEventListener 세 번째 인수로 흐름 결정

## 주요 마우스 이벤트
- onclick, ondblclick, onmousedown, onmouseup, onmouseover, onmouseout 등
...
## 폼 이벤트
- onsubmit: 제출 시 호출, false 리턴 시 취소
- onreset: 초기화 시 호출, false 리턴 시 취소
